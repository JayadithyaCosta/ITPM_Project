...
//boolean value return the success or unsucess status to the servlet from the dbutil
public static boolean updatequestiondetails(String QuestionID,String Question_Statement, String Option_1,String Option_2,
												String Option_3,String Option_4,String Answer)//create parameters to catch the values pass from the servlet
{
	...
	try {
		...
		con = DBconnection.getConnection();
		stmt = con.createStatement();
	    String sql = "UPDATE kanampella_junior.question_details SET Question_Statement = ('"+Question_Statement+"'), Option_1 = ('"+Option_1+"'), Option_2 = ('"+Option_2+"'), Option_3 = ('"+Option_3+"'), Option_4 = ('"+Option_4+"'), Answer = ('"+Answer+"') WHERE QuestionID = '"+QuestionID+"' ";
		int rs = stmt.executeUpdate(sql);
...
//boolean value return the success or unsucess status to the servlet from the dbutil
public static boolean deletequestion(String QuestionID)//create parameters to catch the values pass from the servlet
{
	...
	try {
		...
		con = DBconnection.getConnection();
		stmt = con.createStatement();
		String sql = "delete from  kanampella_junior.question_details WHERE QuestionID = '"+QuestionID+"'";//sql query to delete the value from the database
		int rs = stmt.executeUpdate(sql);
...
//Get Available Exam paper Count
	public static List<exampapercount> countpapers() {
		
		ArrayList<exampapercount> epapercount = new ArrayList<>();
			...
			try {
	    		...
	    		con = DBconnection.getConnection();
	    		stmt = con.createStatement();
	    	    String sql = "select count(ExamID) from kanampella_junior.exam";
	    	    ResultSet rs = stmt.executeQuery(sql);
				...
				while(rs.next()) {
					...
					exampapercount q1 = new exampapercount(count);
					epapercount.add(q1);
